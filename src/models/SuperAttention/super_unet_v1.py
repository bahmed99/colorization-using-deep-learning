...
class percep_vgg19_bn(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(
            *list(vgg19_bn(weights='DEFAULT').features.children())[:-1]
        )
...
class gen_color_stride_vgg16_new(nn.Module):
    ...
    def forward(self, target, ref, luminance_target, label_mask_x, label_mask_ref, target_map, x_real, img_ref_color_norm, device):
        ...
        superattention_4, sim_matrix_4, alpha_4 = self.attention_4(encd_feat4_t, encd_feat4_R, label_mask_x[3], label_mask_ref[3], block4, label_max_4, encd_feat4_rgb_R,label_mask_ref_4 ,device)
        ...
        # sim_all = []
        #
        # sim_all.append(sim_matrix_1)
        # sim_all.append(sim_matrix_2)
        # sim_all.append(sim_matrix_3)
        #
        c, h, w = x_real[:, 0, :, :].shape
        out_unorm = (out_reg * 127.0)
        predictedAB = resize(out_unorm[0, :, :, :].cpu().numpy(), (2, h, w))
